...
use MjNCMS::UserAdminLibWrite;
use MjNCMS::UserSiteLibWrite;
use MjNCMS::UserSiteLibAny;
...
sub users_get ($) {
    ...
    return MjNCMS::UserSiteLibAny::users_get($cfg);
...
sub users_add ($) {
    ...
    if (scalar @{${$cfg}{'extraslaves'}}) {
        ...
        foreach my $m_id (@{${$cfg}{'extraslaves'}}) {
...
sub users_edit ($) {
    ...
    if (
        ...
        unless (&MjNCMS::UserSiteLibWrite::change_email(${$cfg}{'email'}, ${$cfg}{'member_id'})){
            return {
                status => 'fail', 
    ...
    if (${$cfg}{'is_cms_active'} != $mres -> {'is_cms_active'}) {
        unless (
            &MjNCMS::UserSiteLibWrite::set_cms_active(
                ${$cfg}{'is_cms_active'}, 
                        ${$cfg}{'member_id'})
        ){
            return {
    ...
    unless (
        &MjNCMS::UserAdminLibWrite::set_forum_active(
            ${$cfg}{'is_forum_active'},
                    ${$cfg}{'member_id'})
    ){
        return {
...
sub users_delete ($) {
    ...
    &MjNCMS::UserSiteLibWrite::delete_forum_record(${$cfg}{'member_id'});
