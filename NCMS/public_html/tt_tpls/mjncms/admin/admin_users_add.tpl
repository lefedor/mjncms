...
<table border="0" cellspacing="1" cellpadding="4" align="center" width="100%" class="bordercolor">
    ...
    <tr class="windowbg">
        <td class="windowbg nwp lual" style="padding: 7px;"[% IF colspan %] colspan="[% colspan %]"[% END %]>
            <form onSubmit="javascript:return confirm('[% loc('Add user') | html %]?');[% IF SESSION.REQ_ISAJAX %]submit_add_user_frm();return false;[% END %]" name="save_add_user_frm" id="save_add_user_frm" action="[% bytestream(SESSION.ADM_URL, 'url_escape', 'A-Za-z0-9\/\-\.\_\~') %]/users/add" method="post" accept-charset="[% TT_VARS.html_charset %]">
                <table>
                    <tr>
                        <td>
                    ...
                    <tr>
                        <td colspan="2">
                            <input type="submit" value="[% loc('Add user record') | html %]" />
                        </td>
                    </tr>
                </table>
