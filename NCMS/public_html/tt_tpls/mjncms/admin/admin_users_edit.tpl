...
[% IF (matches = TT_VARS.member_id.match('^\d+$')) -%]
...
<table border="0" cellspacing="1" cellpadding="4" align="center" width="100%" class="bordercolor">
    ...
    <tr class="windowbg">
        <td class="windowbg nwp lual" style="padding: 7px;"[% IF colspan %] colspan="[% colspan %]"[% END %]>
            <form onSubmit="javascript:return confirm('[% loc('Edit user') | html %]?');[% IF SESSION.REQ_ISAJAX %]submit_edited_user_frm();return false;[% END %]" name="save_edited_user_frm" id="save_edited_user_frm" action="[% bytestream(SESSION.ADM_URL, 'url_escape', 'A-Za-z0-9\/\-\.\_\~') %]/users/edit/[% member_id %]" method="post" accept-charset="[% TT_VARS.html_charset %]">
                <table>
                    <tr>
                        <td>
                            [% loc('Login') | html %]:
                        ...
                        <td class="lual" rowspan="9">
                            ...
                            [% FOREACH eusr=extra_users -%]
                                [% NEXT UNLESS eusr.member_id #no guest replace -%]
                    ...
                    <tr>
                        <td>
                            <label for="usr_name" title="[% loc('Up to 255 chars') | html %]">[% loc('CMS Name') | html %]: </label>
                        </td>
                        <td>
                    ...
                    <tr>
                        <td class="lual">
                            <label title="[% loc('Guest one will be skipped if multioption') | html %]" for="role_id">[% loc('Role(s)') | html %]: </label>
                        </td>
                        <td>
