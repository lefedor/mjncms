...
[% UNLESS t_usersddata -%]
    ...
    [% END -%]
    [% usersddata=TT_CALLS.get_usersddata({
        'ids' => t_users, 
        'get_all_records' => 1
    }) -%]
    [%# usersddata.q -%]
    [% IF usersddata.message -%]
        Users data not received: [% usersddata.message | html -%]
        [% RETURN -%]
    [% ELSE -%]
        [% usersddata=usersddata.users -%]
    [% END -%]
[% ELSE -%]
    [% usersddata=t_usersddata -%]
[% END -%]
[% IF !(usersddata && usersddata.size) &&  !t_nousr && !t_anyusr -%]
