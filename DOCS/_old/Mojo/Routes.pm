...
sub new {
    ...
    $self->add_condition(
        ...
        host => sub {
            ...
            # Methods
            return unless $hosts && ref $hosts eq 'ARRAY';
        ...
        port => sub {
            ...
            # Methods
            return unless $ports && ref $ports eq 'ARRAY';
        ...
        scheme => sub {
            ...
            # Methods
            return unless $schemes && ref $schemes eq 'ARRAY';
...
sub host {
    ...
    # Methods
    my $hosts = ref $_[0] ? $_[0] : [@_];

    # Shortcut
    return $self unless @$hosts;

    # Condition
    push @{$self->conditions}, host => [map { lc $_ } @$hosts];
...
sub port {
    ...
    # Methods
    my $ports = ref $_[0] ? $_[0] : [@_];

    # Shortcut
    return $self unless @$ports;

    # Condition
    push @{$self->conditions}, port => [map { lc $_ } @$ports];
...
sub scheme {
    ...
    # Methods
    my $schemes = ref $_[0] ? $_[0] : [@_];

    # Shortcut
    return $self unless @$schemes;

    # Condition
    push @{$self->conditions}, scheme => [map { lc $_ } @$schemes];
...
=head2 C<host>
    ...
    $r = $r->host('localhost');
    $r = $r->host(qw/localhost remotehost.com/);
    $r = $r->host([qw/localhost remotehost.com/]);

Apply C<hist> constraint to this route.
...
Apply C<port> constraint to this route.

=head2 C<scheme>

    $r = $r->scheme('http');
    $r = $r->scheme(qw/http https/);
    $r = $r->scheme([qw/http https/]);

Apply C<scheme> constraint to this route.
